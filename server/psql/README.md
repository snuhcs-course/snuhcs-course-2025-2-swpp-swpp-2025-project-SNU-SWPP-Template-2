...
# 2. Get the latest database file from teammate
# Download the shared .sql.gz file to psql/db/ directory

# 3. Import the database (automatically detects compressed files)
cd psql && python settings/team_sync.py import
...
python recommend/recommend.py japanese_lover      # Custom profile
```
...
from recommend.client import RestaurantRecommender, UserProfile
...
```bash
python testing/integration_test_psql.py       # Full test suite
